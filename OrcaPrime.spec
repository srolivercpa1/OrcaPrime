...
a = Analysis([str(root/'run_app.py')], pathex=[str(root)],
    binaries=[], datas=[(str(root/'config'/'client.json'),'config'),(str(root/'assets'/'premium-banner.png'),'assets'),(str(root/'assets'/'brand.png'),'assets'),(str(root/'assets'/'orcaprime-commercial.ico'),'assets'),
...
exe = EXE(pyz, a.scripts, [], exclude_binaries=True, name='OrcaPrime',
    ...
    console=False, icon=str(root/'assets'/'orcaprime-commercial.ico'))
